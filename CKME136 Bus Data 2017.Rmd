...
```{r} 
model.Bus3<- glm(Measure ~ mday+Month+Hour+Minute+Route, data = Bus3, family=binomial)
summary(model.Bus3)
```


## Set up the test set for the classification tree. 
```{r} 
set.seed(101)
alpha <- 0.7
...
class.tree <- rpart(Measure ~ Route+Incident+Location+mday+Month+Hour, data = train.set) 
printcp(class.tree)
